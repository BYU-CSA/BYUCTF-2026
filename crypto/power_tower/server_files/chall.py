...
# Set these to false for the actual challenge
DEBUG = False
PRINT_M = False
...
def get_challenge():
    ...
    # Compute c
    factors = {q:1 for q in p}
    primes = generate_primes_up_to(2**PRIME_BITS)
    phis = compute_phis(factors, NUM_EXPS, primes)
    c = power_tower_mod_n([m] + exps, n, phis)
    if DEBUG:
        assert c == power_tower_mod_n_naive([m] + exps, n, phis[0]), "Failed sanity check"
...
def main():
    ...
    print(f"n = {n}")
    print(f"c = {c}")
    print(f"e = {"^".join([str(e) for e in exponent_list])} (exponents applied right to left)")
    if PRINT_M:
        print(f"m = {m}")
