...
Notice that it's populating `edx` / arg3? It's only taking the hash of 4 bytes, which MEANS that we can brute force just those four bytes and finally have our full flag!! I threw together [brute.py](./brute.py) pretty quickly and ta-da! We have our last chunk: `c0d3`.
